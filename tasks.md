...
## 2b. Handwriting worksheets - two pairs done

Printable A4 workbooks that teach writing, generated by `worksheets/gen_worksheets.py`
and rendered by `worksheets/build_pdfs.sh` (headless Chrome, for correct Devanagari
shaping). Static files, built once, sent on request.

| Pair | Books | Pages | Linked into course |
|---|---|---|---|
| `bengali_to_hindi` | 8 | 169 | [x] stages 0-5 |
| `telugu_to_hindi` | 8 | 169 | [x] stages 0,1,2,3,4,6 |

Books: vowels (14), consonants (41), matras (14), conjuncts (35), words easy (18) and
hard (17), sentences easy (15) and hard (15).

The generator is pair-driven. The Devanagari target data, the ruling geometry and the
font metrics are shared; per pair it carries interface strings, letter equivalents,
glosses and sentence translations.

**The bridges are written per pair, not translated**, because they are not the same:

| | Bengali source | Telugu source |
|---|---|---|
| headline | already drawn last as the মাত্রা | **new** - Telugu has no headline |
| conjuncts | same concept, many identical pairs | same concept, **opposite direction** (ottu below vs half-form beside) |
| ड़ / ढ़ | familiar from ড় ঢ় | **new sounds** |
| short e/o | neither writes them | Telugu has ఎ ఒ, Hindi has only the long forms |

- [ ] more pairs: any Devanagari target needs only a `LANGS` entry plus equivalents,
      glosses and sentence translations. A different target script also needs the three
      font-metric ratios remeasured with `calibrate_font.html`.
